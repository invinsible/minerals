...
require_once '/home/b/bicinform/gfcctv.beget.tech/core/config/config.inc.php';
...
while (($csv = fgetcsv($handle, 0, $delimeter)) !== false) {
	...
	if ($exists = $modx->getObject($data['class_key'], $q)) {
		...
		if (!$update) {
			...
			$tvsCustom = $exists->getMany('TemplateVarResources');		
			foreach($tvsCustom as $tv){
				$someTvId = 'tv'.$tv->get('tmplvarid');	
				if(!strpos($fields, $someTvId)) {
					$data[$someTvId] = $tv->get('value');
				}								
			}
			$exists->clearCache();
		}
